 ...
 */
public class ShellJavaCommandInCentos {
...
//        String[] envVars = {"D:\\InstallSoftware\\Git\\bin\\","D:\\InstallSoftware\\Git\\usr\\bin"};
//        String[] envVars = {"/bin/sh", "-c", "ls -l"};
//        String[] envVars = {"D:\\InstallSoftware\\Git\\bin\\bash", "-c", "ls -l | sort -k 8 | head -n 3 | awk -F ' ' '{print $9}' "};
//        String[] envVars = {"D:\\InstallSoftware\\Git\\bin\\bash", "-c", "pwd | ls && cd java-small-app | pwd | ls -l"};
        String[] envVars = {"D:\\InstallSoftware\\Git\\bin\\bash", "-c", "pwd;ls;cd java-small-app;pwd;ls -l;clear;"};

        //exec方法第一个参数是执行的命令，第二个参数是环境变量，第三个参数是工作目录
//        Process pr = rt.exec("cmd /c javac SqlExample.java && java a", envVars, new File("D://"));
        // C:\Windows\System32
//        Process pr = rt.exec("/usr/bin javac SqlExample.java && java a", envVars, new File("D://"));
        Process pr = rt.exec(envVars);
