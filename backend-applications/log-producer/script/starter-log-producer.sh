...
#java -cp ./lib/ngaa-cdn-1.0.0.18-alpha.jar  com.ngaa.bigdata.demo.HiveJDBCDemoi
...
#java -cp "lib/log-producer-0.0.6-snapshot.jar:lib/log4j-1.2.12.jar:lib/slf4j-api-1.7.2.jar:lib/slf4j-log4j12-1.7.2.jar"  cn.tiakon.logproducer.core.LogProducerStarter  producer-conf.properties

#>> logs/java-app-$(date +"%Y%m%d").log 2>&1 &